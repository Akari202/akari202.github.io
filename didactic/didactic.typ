...
#let extract-title(file-path, default-name) = {
  ...
  let title-match = content.match(regex("(?:#show\: minimal-setup|#show\: setup|#set document)(?:[^)]*?)\btitle:\s*\"([^\"]+)\""))
    ...
    for path in tree-data {
      ...
      if path.starts-with(target-dir) {
        ...
        if parts.len() == 1 {
          ...
          level-dict.insert(title, path.replace(regex("^content\/"), "").replace(regex("\.typ$"), "").replace("/", "-"))
        }
        else if parts.len() > 1 {
...
if i.ends-with("typ") {
    ...
    [
    #document(output.replace(regex("typ$"), "html"), format: "html", author: "Akari202")[
#html.head()[
#html.link(rel: "stylesheet", type: "text/css", href: "https://cdn.jsdelivr.net/gh/dreampulse/computer-modern-web-font@master/fonts.css")
#html.link(rel: "stylesheet", type: "text/css", href: "/style.css")
#html.link(rel: "icon", type: "image/x-icon", href: "/favicon.ico")
#html.link(rel: "author", href: "/humans.txt")
#html.meta(name: "robots", content: "index, follow, nosnippet, nocache, noarchive, noimageindex")
]
// #html.div(class: "inner-body")[
...
{
        i.pairs().map(j => {
            if j.at(1) == output.replace(regex("\.typ$"), "").replace("/", "-") {
...
#html.footer()[
#html.hr()
#html.elem("small", attrs: (style: "display: flex; justify-content: space-between;"))[
#html.span()[
